class Nebula
{
  ...
  testCloud()
  {
    this.paintsys.push(new Paint(createVector(width/2, height/2), 50, 50, 100, 100, 255,0,0,true));
  }

  createSpecificCloud(low, high, size, iter, prolif, r,g,b, isStroke)
  {
    let xoff = 0.0;
    for (let x = 0; x < width; x+=2) {
      let yoff = 0.0;
      for (let y = 0; y < height; y+=2) {
        if (random(0,1000) < prolif)
          ...
          if (nois > low && nois < high)
            {
              this.paintsys.push(new Paint(createVector(x, y), 0, 0, size, iter, r,g,b,isStroke));
            }
          }
        yoff += 0.01;
      }
          ...
          if (nois >= threshold)
            {
              ...
              let iter = map(nois,threshold,0.8,1, 20, true);
              this.paintsys.push(new Paint(createVector(x, y), distance, angle, 30*mod, iter, r,g,b,true));
            }
          }
        yoff += 0.01;
      }
